...
| 桌面宠物 | 狸花猫会在拖动时朝对应方向走路，并跟随识图、翻译、完成、失败和朗读状态切换动作；紧凑气泡可直接显示、复制和朗读译文。宠物采用独立资源包，可替换和扩展。 |
...
| 打开设置或退出 | 右键系统托盘图标 |
| 打开完整翻译结果 | 双击桌面宠物 |
| 移动或管理宠物 | 左键水平拖动时播放左右走路动作；右键打开宠物菜单 |
...
在“设置 → 桌面宠物”中可以开关宠物、选择角色、将大小调整为 `70%–140%`，以及控制结果气泡。宠物位置会按 Windows 全局逻辑坐标保存，支持多显示器与负坐标副屏。水平拖动宠物时会按方向播放 8 帧走路动作；右键菜单可以手动触发舔爪、打招呼、眨眼或跳跃。闲置 14–30 秒后，宠物也会随机舔爪或挥手。

宠物会随翻译流程自动行动：OCR 时警觉等待，翻译时进入扫描思考循环，完成后审阅结果，失败时显示低落反馈。关闭宠物后，后续翻译任务不会让宠物或结果气泡意外重新出现。
